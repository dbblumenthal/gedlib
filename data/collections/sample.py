...
group.add_argument("--size", help="size of sample; must be between 0 and the size of the dataset", type=int)
...
if args.size_ratio:
	...
	if args.size < 0 or args.size > total_candidate_size:
		raise Exception("SIZE must be between 0 and the size of the input dataset")
	if args.balanced:
		sample_sizes = {cl : min(min_candidate_sizes, int(args.size / num_classes)) for cl in classes}
	else:
		sample_sizes = {cl : int(candidate_sizes[cl] * (float(args.size) / total_candidate_size)) for cl in classes}
