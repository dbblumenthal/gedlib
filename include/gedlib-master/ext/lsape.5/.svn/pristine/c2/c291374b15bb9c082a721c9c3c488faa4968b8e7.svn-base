...
void allSolLSAP_helper(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], const mxClassID &mxC)
{
  ...
  unsigned int n = nrows, m = ncols, n1 = nrows-1; //m1 = m-1;
  if (nrhs == 5) { n = (unsigned int)mxGetScalar(prhs[3]); m = (unsigned int)mxGetScalar(prhs[4]); n1 = n+m; }
  
  std::list<unsigned int*> solutions;
  DT minCost = lsapeSolutions<DT>(C, n, m, k, solutions, lsape_model);
...
{
  if (nrhs < 2 || nrhs > 5 || nlhs < 1 || nlhs > 2) mexErrMsgTxt("USE : [sol,minCost] = lsapeSolutions(C,ksol,lsape_model=0,nrows,ncols)");
