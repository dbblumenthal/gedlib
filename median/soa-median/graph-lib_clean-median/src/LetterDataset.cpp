...
LetterDataset::LetterDataset(const char* filename)
{
...
void LetterDataset::loadDS(const char* filename)
{
  ...
  if (f_tmp.is_open()){
    ...
    while (getline(f_tmp, s)){
      if (s[0] != '#'){
	      ...
	      liness >> ctfile;
	      int y;
	      //liness >> y; former version
	      y = (int)ctfile[0];
	      std::string full_ctfile = path_ctfile;
	      full_ctfile += ctfile;
	      LetterGraph * g = new LetterGraph(full_ctfile.c_str());
	      this->add(g,y);
      }
    }
  }
