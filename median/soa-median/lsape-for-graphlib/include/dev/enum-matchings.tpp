...
cDigraph<IT> greedySortDigraph(const DT *C, const IT &nrows, const IT &ncols){
  ...
  for (IT nbc = 0; pt_idx != pt_idxs_end && nbc < nrows+ncols-2; pt_idx++)
  {
    ...
    if (i == nrows-1) { if (varrho[j] == nrows) { varrho[j] = i; approx += C[*pt_idx]; nbc++; dg(j+n,j) = 1; } else dg(j+n,j)=-1; }
    else
      if (rho[i] == ncols)
        if (j == ncols-1) { rho[i] = j; approx += C[*pt_idx]; nbc++; dg(i,i+m) = 1;}
        else
        {
          if (varrho[j] == nrows) { rho[i] = j; varrho[j] = i; approx += C[*pt_idx]; nbc += 2; dg(i,j) = 1;}
          else dg(i,j) = -1;
        }
      else
