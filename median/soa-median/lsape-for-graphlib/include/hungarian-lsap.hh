...
// --------------------------------------------------------------------------------
template <class DT, typename IT>
void hungarianLSAP(const DT *C, const IT &n, const IT &m, IT *rho, DT *u, DT *v, IT *varrho = NULL, unsigned short init_type = 1, bool forb_assign = false, bool neg_val = false)
{
  if (neg_val) // translate by -min
  {
    DT mnv = std::numeric_limits<DT>::max();
    int i = 0, nm = n*m;
    const DT *cit = C;
    for (; i < nm; i++, ++cit) if (*cit < mnv) { mnv = *cit; }
    if (mnv < 0)
    {
      DT *Ct = new DT[nm];
      DT *ctit = Ct;
      cit = C;
      mnv = -mnv;
      for (i = 0; i < nm; i++, ++ctit, ++cit) *ctit = *cit + mnv;
      if (n == m) hungarianSquareLSAP<DT,IT>(Ct,n,rho,u,v,varrho,init_type,forb_assign);
      else hungarianRectLSAP<DT,IT>(Ct,n,m,rho,varrho,u,v,init_type,forb_assign);
      delete[] Ct;
      return;
    }
    else { /* std::cerr << "hungarianLSAP without neg_val" << std::endl; */ }
  }
