...
namespace lsape {
  ...
  */
   ...
   *  \param[in] C nrows*ncols array representing an edit cost matrix of size nrows x ncols
   *  \param[in] nrows number of rows (last row corresponds to the null element), we have n+1=nrows
   *  \param[in] ncols number of colums (last column corresponds to the null element), we have m+1=ncols
   *  \param[out] Cext (nrows-1+ncols-1)x(ncols-1+nrows-1) extended cost matrix (must be previously allocated with size nrows*ncols)
   *  \param[in] valmx value for outer diagonal elements of Cext (representing forbidden assignments)
   */
  template <typename DT>
  ...
  // lsape2lsap
  template <typename DT>
    DT* lsape2lsapInst(const DT *C, const LSAPE_IndexType &nr, const LSAPE_IndexType &nc, enum LSAPE_MODEL lsape_model = EBP, DT *Cout = NULL)
    {
      switch (lsape_model)
      {
      case EBP: break;
    ...
    void lsapeSolverModel(const DT *C, const LSAPE_IndexType &nr, const LSAPE_IndexType &nc, LSAPE_IndexType *rho, LSAPE_IndexType *varrho, DT *u, DT *v, enum LSAPE_MODEL lsape_model = ECBP, unsigned short init_type = 1)
  {
      ...
      case FLWC: case FLCC: case FBP: case FBP0:
	{
	  ...
	  for (; pt != pte; ++pt) if (*pt < minC) minC = *pt;
	  if (minC < 0) for (pt = C; pt != pte; ++ptCt, ++pt) *ptCt = *pt - minC;
	  else  for (pt = C; pt != pte; ++ptCt, ++pt) *ptCt = *pt;
	  // solve
