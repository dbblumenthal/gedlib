/*!
 * @file  bipartite.ipp
 * @brief Bipartite class definition.
 */

#ifndef SRC_METHODS_BIPARTITE_IPP_
#define SRC_METHODS_BIPARTITE_IPP_

namespace ged {

template<class UserNodeLabel, class UserEdgeLabel>
Bipartite<UserNodeLabel, UserEdgeLabel>::
~Bipartite() {}
	...
	for (std::size_t row_in_master = 0; row_in_master < master_problem.num_rows(); row_in_master++) {
		for (std::size_t col_in_master = 0; col_in_master < master_problem.num_cols(); col_in_master++) {
			if ((row_in_master < g.num_nodes()) and (col_in_master < h.num_nodes())) {
				master_problem(row_in_master, col_in_master) = compute_substitution_cost_(g, h, g_ids_to_nodes.at(row_in_master), h_ids_to_nodes.at(col_in_master));
			}
...
compute_substitution_cost_(const GEDGraph & g, const GEDGraph & h, GEDGraph::NodeID i, GEDGraph::NodeID k) const {
	...
	for (auto ij = g.incident_edges(i).first; ij != g.incident_edges(i).second; ij++, j++) {
		subproblem(j, h.degree(k)) = this->ged_data_.edge_cost(g.get_edge_label(*ij), ged::dummy_label());
	}

	// Collect edge insertion costs.
	std::size_t l{0};
	for (auto kl = h.incident_edges(k).first; kl != h.incident_edges(k).second; kl++, l++) {
		subproblem(g.degree(i), l) = this->ged_data_.edge_cost(ged::dummy_label(), h.get_edge_label(*kl));
	}
	j = 0;
	...
	for (auto ij = g.incident_edges(i).first; ij != g.incident_edges(i).second; ij++, j++) {
		l = 0;
		for (auto kl = h.incident_edges(k).first; kl != h.incident_edges(k).second; kl++, l++) {
			subproblem(j, l) = this->ged_data_.edge_cost(g.get_edge_label(*ij), h.get_edge_label(*kl));
		}
	}
