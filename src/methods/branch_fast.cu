...
namespace ged {
    ...
    prepare_cuda_env_(const ged::DMatrix &mat,
        ...
        // Tricky 有空修
        // 可以改异步io
        if (device_sorted_edge_labels_g == nullptr) {
        ...
        if (device_sorted_edge_labels_h == nullptr) {
    ...
    __global__ void
    compute_deletion_cost(ged::GEDGraph::NodeID k, double &cost) {
        ...
        if (tid == 0) {
            cost = reduction_helper[0];
        }
    }

    __global__ void
    compute_insertion_cost(ged::GEDGraph::NodeID i, double &cost) {
        extern __shared__ double reduction_helper[];
    ...
    __global__ void
    compute_relabelling_cost(ged::GEDGraph::NodeID i, ged::GEDGraph::NodeID k, double &cost) {
        extern __shared__ double reduction_helper[];
    ...
    __global__ void
    compute_multiset_intersection_size(ged::GEDGraph::NodeID i, ged::GEDGraph::NodeID k, unsigned &size) {
        // Worst case: O(n)
    ...
    compute_substitution_cost_with_cuda(
        ...
        double min_insertion_cost{};
        double min_deletion_cost{};
        double min_relabling_cost{};
        ...
        <<<g_num_nodes, h_num_nodes, g_num_nodes * h_num_nodes * sizeof(double), relabeling>>>
                (i, k, min_relabling_cost);

        // Compute multiset intersection size.
        compute_multiset_intersection_size<<<1, thrust::min(sg, sh)>>>(i, k, intersection_size);

        // Write Back
        // TODO fuck off
        cost += static_cast<double>((h_deg - g_deg) * min_insertion_cost * 0.5);
        cost += static_cast<double>((g_deg - h_deg) * min_deletion_cost * 0.5);
        if(thrust::min(h_deg, g_deg) - intersection_size > 0) {
            static_cast<double>(thrust::min(g_deg,h_deg) - intersection_size) * min_relabling_cost * 0.5;
        }
    ...
    extern "C"
    __host__  double *
    launch_kernel(int g_num_nodes, int h_num_nodes) {
