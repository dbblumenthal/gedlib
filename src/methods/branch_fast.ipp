    ...
    lsape_parse_option_(const std::string &option, const std::string &arg) {
        if (option == "sort-method") {
            if (arg == "STD") {
                ...
                throw Error(std::string("Invalid argument \"") + arg +
...
#pragma omp parallel for if(this->num_threads_ > 1)
#endif
        // center of the master calculated by gpu
        for (std::size_t row_in_master = 0; row_in_master < master_problem.num_rows(); row_in_master++) {
            for (std::size_t col_in_master = 0; col_in_master < master_problem.num_cols(); col_in_master++) {
                if ((row_in_master < g.num_nodes()) and (col_in_master < h.num_nodes())) {
                    master_problem(row_in_master,col_in_master) =
                            this->ged_data_.node_cost(g.get_node_label(row_in_master), h.get_node_label(col_in_master));
        ...
        for (std::size_t row_in_master = 0; row_in_master < master_problem.num_rows(); row_in_master++) {
            for (std::size_t col_in_master = 0; col_in_master < master_problem.num_cols(); col_in_master++) {
                if ((row_in_master < g.num_nodes()) and (col_in_master < h.num_nodes())) {
                    master_problem(row_in_master,col_in_master) += mat[row_in_master * g.num_nodes() + col_in_master];
                }
            }
        }
...
#pragma omp parallel for if(this->num_threads_ > 1)
        ...
        for (std::size_t row_in_master = 0; row_in_master < master_problem.num_rows(); row_in_master++) {
            for (std::size_t col_in_master = 0; col_in_master < master_problem.num_cols(); col_in_master++) {
                if ((row_in_master < g.num_nodes()) and (col_in_master < h.num_nodes())) {
                    master_problem(row_in_master, col_in_master) = compute_substitution_cost_(
                            g, h, row_in_master,
                            col_in_master,
                            sorted_edge_labels_g,
                            sorted_edge_labels_h);
                } else if (row_in_master < g.num_nodes()) {
                    master_problem(row_in_master, h.num_nodes()) = compute_deletion_cost_(g, row_in_master);
                } else if (col_in_master < h.num_nodes()) {
                    master_problem(g.num_nodes(), col_in_master) = compute_insertion_cost_(h, col_in_master);
    ...
    template<class UserNodeLabel, class UserEdgeLabel>
    DMatrix &BranchFast<UserNodeLabel, UserEdgeLabel>::get_data() {
        return this->ged_data_;
    }
...
// === Definition of private class SortedUserEdgeLabels_. ===
    ...
    template<class UserNodeLabel, class UserEdgeLabel>
    const std::map<GEDGraph::NodeID, std::vector<LabelID>> &
    BranchFast<UserNodeLabel, UserEdgeLabel>::
    SortedEdgeLabels_::get_data() const {
        return this->sorted_edge_labels_;
    }
...
// === Definition of private helper functions. ===
    ...
    compute_substitution_cost_(const GEDGraph &g, const GEDGraph &h, GEDGraph::NodeID i, GEDGraph::NodeID k,
        ...
        // 把incident_labels向量传过去reduce
        // 应该可以 用vector直接存一整个
        // sort部分存在优化空间 不是现在
        if (g.degree(i) < h.degree(k)) {
            double min_edge_ins_cost{std::numeric_limits<double>::infinity()};
            for (auto label_h = sorted_edge_labels_h.get_incident_labels(k).begin();
                 label_h != sorted_edge_labels_h.get_incident_labels(k).end(); label_h++) {
        ...
        if (g.degree(i) > h.degree(k)) {
            ...
            for (auto label_g = sorted_edge_labels_g.get_incident_labels(i).begin();
                 label_g != sorted_edge_labels_g.get_incident_labels(i).end(); label_g++) {
        ...
        while ((label_g != sorted_edge_labels_g.get_incident_labels(i).end()) and
               (label_h != sorted_edge_labels_h.get_incident_labels(k).end())) {
            if (*label_g == *label_h) {
                intersection_size++;
                label_g++;
                label_h++;
        ...
        if (std::min(g.degree(i), h.degree(k)) - intersection_size > 0) {
            cost += static_cast<double>(std::min(g.degree(i), h.degree(k)) - intersection_size) * min_edge_subs_cost *
                    0.5;
        }
