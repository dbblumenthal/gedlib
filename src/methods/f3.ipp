...
/*!
 * @file f3.ipp
 * @brief ged::F3 class definition.
 */

#ifndef SRC_METHODS_F3_IPP_
#define SRC_METHODS_F3_IPP_


namespace ged {

template<class UserNodeLabel, class UserEdgeLabel>
F3<UserNodeLabel, UserEdgeLabel>::
~F3() {}
...
mip_populate_model_(const GEDGraph & g, const GEDGraph & h, GRBModel & model) {
	...
	std::pair<GEDGraph::EdgeID, std::pair<GEDGraph::NodeID, GEDGraph::NodeID>> key_y;
	...
	for (auto e = g.edges().first; e != g.edges().second; e++, counter_e++) {
		...
		for (auto f = h.edges().first; f != h.edges().second; f++, counter_f++) {
			...
			key_y.second.first = h.tail(*f);
			key_y.second.second = h.head(*f);
			y_[key_y] = model.addVar(0, 1, sub_cost, variable_type_());
			key_y.second.first = h.head(*f);
			key_y.second.second = h.tail(*f);
			y_[key_y] = model.addVar(0, 1, sub_cost, variable_type_());
		}
	}
	...
	// Add node-node constraints to the model.
	for (auto i = g.nodes().first; i != g.nodes().second; i++) {
		key_x.first = *i;
		for (auto k = h.nodes().first; k != h.nodes().second; k++) {
			key_x.second = *k;
			key_y.second.first = *k;
			lhs = 0;
			for (auto e = g.incident_edges(*i).first; e != g.incident_edges(*i).second; e++) {
				key_y.first = *e;
			...
			lhs -= x_.at(key_x) * static_cast<double>(std::min(g.degree(*i), h.degree(*k)));
