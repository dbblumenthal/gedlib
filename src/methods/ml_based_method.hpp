 ...
 * | <tt>\--ground-truth-method ANCHOR_AWARE_GED\|%F2\|%F3\|COMPACT_MIP\|%IPFP</tt> | method for computing the ground truth | @p %IPFP | the methods %F2, %F3, and COMPACT_MIP are available only if GEDLIB is installed with Gurobi |
 * | <tt>\--ground-truth-options '[--@<option@> @<arg@>] [...]'</tt> | options string passed to the ground truth method | @p '' | ged::AnchorAwareGED, ged::F2, ged::F3, ged::CompactMIP, ged::IPFP |
