...
void init_method_name_map(std::map<std::size_t, std::string> &method_name_map) {
    ...
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::BRANCH_FAST) +
                    centrality()] = "branch_fast_pagerank";
    ...
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::RING) + lsape_optimal() +
                    centrality()] = "ring_lsape_optimal_pagerank";
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::RING) + gamma()] = "ring_multisets";
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::RING) + gamma() +
                    centrality()] = "ring_multisets_pagerank";
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::RING) + lsape_greedy()] = "ring_lsape_greedy";
    method_name_map[static_cast<std::size_t>(ged::Options::GEDMethod::RING) + lsape_greedy() +
                    centrality()] = "ring_lsape_greedy_pagerank";*/
}
...
void run_single_test(ged::GEDEnv<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel> &env, ged::GEDGraph::GraphID g_id,
                     ged::GEDGraph::GraphID h_id,
                     ged::Options::GEDMethod method, std::size_t centrality, std::size_t gamma,
                     const std::string &options,
                     std::map<std::size_t, double> &avg_time_in_sec, std::map<std::size_t, double> &avg_ub,
...
void run_tests_on_dataset(const std::string &dataset) {
    ...
    std::vector<ged::GEDGraph::GraphID> graph_ids(
            env.load_gxl_graphs(std::string("../../../data/datasets/") + dataset + "/",
                                std::string("../../../data/collections/") + dataset + ".xml"));
    if (dataset == "GREC") {
    ...
    for (auto g_id: graph_ids) {
        ...
        for (auto h_id: graph_ids) {
            ...
            /*run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BIPARTITE, no_centrality(), no_led_option(),
                            threads_option, avg_time_in_sec, avg_ub, best_ub, ub, distance_to_closest_graph,
                            closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BIPARTITE, centrality(), no_led_option(),
                            threads_option + " " + centrality_option, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);*/
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BRANCH, no_centrality(), no_led_option(),
                            threads_option, avg_time_in_sec, avg_ub, best_ub, ub, distance_to_closest_graph,
                            closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BRANCH, centrality(), no_led_option(),
                            threads_option + " " + centrality_option, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BRANCH_FAST, no_centrality(), no_led_option(),
                            threads_option, avg_time_in_sec, avg_ub, best_ub, ub, distance_to_closest_graph,
                            closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::BRANCH_FAST, centrality(), no_led_option(),
                            threads_option + " " + centrality_option, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            /*run_single_test(env, g_id, h_id, ged::Options::GEDMethod::WALKS, no_centrality(), no_led_option(),
                            walks_depth_option, avg_time_in_sec, avg_ub, best_ub, ub, distance_to_closest_graph,
                            closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::WALKS, centrality(), no_led_option(),
                            centrality_option + " " + walks_depth_option, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::SUBGRAPH, no_centrality(), no_led_option(),
                            threads_option + " " + subgraph_depth_option, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::SUBGRAPH, centrality(), no_led_option(),
                            threads_option + " " + centrality_option + " " + subgraph_depth_option, avg_time_in_sec,
                            avg_ub, best_ub, ub, distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, no_centrality(), lsape_optimal(),
                            threads_option + " " + ring_option_lsape_optimal, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, centrality(), lsape_optimal(),
                            threads_option + " " + ring_option_lsape_optimal + " " + centrality_option, avg_time_in_sec,
                            avg_ub, best_ub, ub, distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, no_centrality(), gamma(),
                            threads_option + " " + ring_option_gamma, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, centrality(), gamma(),
                            threads_option + " " + ring_option_gamma + " " + centrality_option, avg_time_in_sec, avg_ub,
                            best_ub, ub, distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, no_centrality(), lsape_greedy(),
                            threads_option + " " + ring_option_lsape_greedy, avg_time_in_sec, avg_ub, best_ub, ub,
                            distance_to_closest_graph, closest_graph);
            run_single_test(env, g_id, h_id, ged::Options::GEDMethod::RING, centrality(), lsape_greedy(),
                            threads_option + " " + ring_option_lsape_greedy + " " + centrality_option, avg_time_in_sec,
                            avg_ub, best_ub, ub, distance_to_closest_graph, closest_graph);*/
        ...
        for (auto &kv: classification_ratio) {
            auto t = env.get_graph_class(closest_graph.at(kv.first));
            bool res = t == env.get_graph_class(g_id);
            kv.second += res ? 1.0 : 0.0;
        }
    }
    std::cout << "\n";
    ...
    for (auto &kv: method_names) {
        outfile << kv.second << "," << avg_time_in_sec.at(kv.first) << "," << avg_ub.at(kv.first) << ","
                << classification_ratio.at(kv.first) << "," << avg_dev_best_ub.at(kv.first) << "\n";
    }
    outfile.close();
}
